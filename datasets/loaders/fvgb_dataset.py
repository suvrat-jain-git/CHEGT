"""FVGB (Fine-grained Video Gait Benchmark) Dataset Loader.
...
class FVGBDataset(Dataset):
    """PyTorch Dataset for FVGB.

    Structurally mirrors CCVIDDataset with an additional ``condition``
    ...
    def __init__(self, cfg: DictConfig, split: str = "train") -> None:
        ...
        self.transform: GaitTransform = get_transforms(
            ds_cfg, split=("train" if split == "train" else "val")
        )

        allowed_conditions: Optional[List[str]] = list(split_cfg.conditions) \
            if split_cfg.get("conditions") else None
        ...
        for subj_dir in sorted(self.root.iterdir()):
            ...
            for cond_dir in sorted(subj_dir.iterdir()):
                ...
                for seq_dir in sorted(cond_dir.iterdir()):
        ...
        if not self.samples:
            raise RuntimeError(
                ...
                "Check your config paths and conditions."
            )
    ...
    def _load_frames(self, frame_paths: List[Path]) -> torch.Tensor:
        ...
        frames = [Image.open(strided[i]).convert("RGB") for i in indices]
    ...
    def get_meta(self, idx: int) -> Dict[str, Any]:
        s = self.samples[idx]
        return {"subject_id": s["subject_id"], "seq_id": s["seq_id"], "condition": s["condition"]}
